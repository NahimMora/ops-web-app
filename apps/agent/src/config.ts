...
const schema = z.object({
  ...
  OPS_R2_PREVIEW_PREFIX: z.string().regex(/^[a-zA-Z0-9/_-]+$/).default("ops/xvideo-previews"),
...
export const agentConfig = {
  ...
  r2: {
    ...
    endpoint: raw.R2_S3_ENDPOINT || (raw.R2_ACCOUNT_ID ? `https://${raw.R2_ACCOUNT_ID}.r2.cloudflarestorage.com` : undefined),
    bucket: raw.R2_BUCKET,
    publicBaseUrl: raw.R2_PUBLIC_BASE_URL?.replace(/\/$/, ""),
    region: raw.R2_REGION,
    prefix: raw.OPS_R2_VIDEO_PREFIX.replace(/^\/+|\/+$/g, ""),
    previewPrefix: raw.OPS_R2_PREVIEW_PREFIX.replace(/^\/+|\/+$/g, ""),
  },
};
