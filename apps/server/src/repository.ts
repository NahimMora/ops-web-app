...
export interface Repository {
  ...
  retryCommand(id: string, payloadOverride?: Record<string, unknown> | null): Promise<CommandInternal | null>;
...
export class MemoryRepository implements Repository {
  ...
  async retryCommand(id: string, payloadOverride?: Record<string, unknown> | null) { const c = this.commands.get(id); if (!c || !["failed", "waiting_manual_retry", "requires_attention", "cancelled"].includes(c.status)) return null; Object.assign(c, { status: "queued", currentStage: "queued", progressPercent: 0, errorCode: null, errorMessage: null, retryable: false, completedAt: null, assignedAgentId: null, leaseTokenHash: null, leaseExpiresAt: null, sideEffectStarted: false, updatedAt: nowIso() }, payloadOverride ? { payload: clone(payloadOverride) } : {}); return clone(c); }
