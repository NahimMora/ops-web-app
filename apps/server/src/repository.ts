...
export class MemoryRepository implements Repository {
  ...
  async finishCommand(id: string, leaseHash: string, status: CommandStatus, patch: UpdateInput) { const c = this.owned(id, leaseHash); if (!c || !["claimed", "running"].includes(c.status) || !isTerminal(status)) return null; Object.assign(c, normalizePatch(patch), { status, progressPercent: status === "completed" ? 100 : patch.progressPercent ?? c.progressPercent, completedAt: nowIso(), updatedAt: nowIso(), leaseExpiresAt: null, leaseTokenHash: null }); return clone(c); }
